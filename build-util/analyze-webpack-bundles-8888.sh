...
echo "))> Navigating to the InfoMapper Builder main infomapper-builder/ directory."
cd "${mainFolder}" || exit 1
echo "))> Building the InfoMapper Builder using the --stats-json flag."
npm run build:stats
echo ""
echo "))> Done."

echo "))> Navigating to the InfoMapper Builder dist/infomapper-builder/ directory."
cd "${mainFolder}"/dist/infomapper-builder || exit 1
